...
function badge_count_task() {
    ...
    return badge_count_no_finish_task() + badge_count_dept_task();
}
...
function badge_count_ys_task() {
	...
	if (!empty($task_list)) {
            ...
            $where['company_id'] = array('eq', session('company_id'));
		} else {
			return 0;
		}
	

	$task_ys_count = M("Task") -> where($where) -> count();
	return $task_ys_count;
}
...
function list_to_tree1($list, $root = 1, $pk = 'id', $pid = 'pid', $child = '_child') {
	// 创建Tree
	$tree = array();
	if (is_array($list)) {
		// 创建基于主键的数组引用
...
function getqid($pid) {
		  ...
		  $dlist= $dept -> where($where) -> select();

         
          //var_dump($dlist);
		  //var_dump($dlist[0]["pid"]);

         
		  
		  if($dlist[0]['pid']!='1') {

			  $str=getqid($dlist[0]['pid']);
