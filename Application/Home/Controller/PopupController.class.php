...
class PopupController extends HomeController {
	...
	private function _dept_list() {
		...
		if (session('company_id')==1) {
               ...
               $list = $model -> where('is_del=0') -> field('id,pid,name,company_id') -> order('sort asc') -> select();
			   $list = list_to_tree($list);
		}else{
              
			   $where['is_del']=0;
			   $where['company_id']=session('company_id');
			   $list = $model -> where($where) -> field('id,pid,name,company_id') -> order('sort asc') -> select();
			   $list = list_to_tree1($list);
