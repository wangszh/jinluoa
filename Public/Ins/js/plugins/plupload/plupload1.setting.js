$(function() {
	var uploader = new plupload.Uploader({
		...
		init : {
			PostInit : function() {
				if ($("#uploader1 .tbody").length > 0) {
					$("#uploader1 .tbody .loading").css("width", "100%");
					$("#uploader1 .thead").show();
					$("#uploader1 .tbody").each(function() {
			...
			FileUploaded : function(up, file, data) {
				...
				if (myObject.status) {
					if ($("#add_file1").length != 0) {
						$("#add_file1").val($("#add_file1").val() + myObject.sid + ";");
					}
					$("#" + file.id).attr("add_file1", myObject.sid);
					$new_upload = $("#file_list1").attr("new_upload");
					$("#file_list1").attr("new_upload", $new_upload + myObject.sid + ";");
...
$(document).on("click", "#uploader1 a.del", function() {
	$obj = $(this).parents("li");
	id = $obj.attr("id");
	//file = uploader.getFile(id);
	//ui_confirm("确定要删除吗？", function() {
		$add_file = $obj.attr("add_file1");
		$new_upload = $("#file_list1").attr("new_upload");
		$("#add_file1").val($("#add_file").val().replace($add_file + ";", ""));
		$("#file_list1").attr("new_upload", $new_upload.replace($add_file + ";", ""));
